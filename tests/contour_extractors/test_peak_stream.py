...
class TestPeakStreamHelper(unittest.TestCase):
    ...
    def test_get_contour(self):
        ...
        psh.get_contour()
        actual_contour_idx = psh.contour_idx
        expected_contour_idx = [3, 1, 5, 7, 4]
        self.assertEqual(expected_contour_idx, actual_contour_idx)
    ...
    def test_peak_streaming(self):
        ...
        expected_c_numbers = np.array([0, 0, 1, 1, 1])
        expected_c_times = np.array([0.1, 0.05, 0.15, 0.2, 0.05])
        expected_c_freqs = np.array([
            4070.7809073345124, 4037.4869187550275, 4103.4466698165415,
            4135.5074758674773, 4103.4466698165415
        ])
        expected_c_sal = np.array([0.3, 1.0, 0.2, 0.2, 0.1])
